#!/usr/bin/env node
/**
 * Import whatsapp-bot/src/data/products.json into PostgreSQL.
 *
 * Prerequisites:
 *   1. Postgres running (docker compose -f docker-compose.db.yml up -d)
 *   2. DATABASE_URL set in whatsapp-bot/.env
 *
 * Usage (from whatsapp-bot/ — so pg + dotenv resolve):
 *   npm run db:seed
...
const SCHEMA = path.join(ROOT, "whatsapp-bot", "db", "schema.sql");

dotenv.config({ path: BOT_ENV });
