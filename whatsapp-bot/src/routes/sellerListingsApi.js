...
/** POST /api/seller/listings/generate — approved seller + photo → AI draft */
router.post("/generate", async (req, res) => {
  const { phone, imageBase64, mimeType = "image/jpeg", caption = "" } = req.body || {};
  ...
  try {
    ...
    const draft = await generateSellerListingDraft(buffer, mimeType, caption);
    res.json({ draft, seller: { id: check.supplier.id, businessName: check.supplier.businessName } });
  } catch (err) {
