...
function formatProductAck(product, { updated = false, awaitingPhoto = false, imageAutoGenerated = false } = {}) {
  ...
  const photoHint = awaitingPhoto
    ? imageAutoGenerated
      ? `\n📷 _Photo auto-generated. Send your own product photo to replace it._`
      : `\n📷 _Generating product photo… send your own photo to attach to \`${product.id}\`_`
    : product.imageUrl
      ? ""
...
/** Fetch or AI-generate a product photo when admin adds by text without an image. */
async function enrichStoreProductImage(product) {
  const mod = await import(pathToFileURL(PRODUCT_IMAGES_MODULE).href);
  const result = await mod.enrichProductImage(product, {
    serperKey: process.env.SERPER_API_KEY || "",
    log: (msg) => console.log("[catalog-admin] image:", msg.trim()),
  });
  product.imageUrl = result.path;
  product.imageKey = mod.imageKeyForName(product.name);
  return result;
}
...
async function upsertStoreProduct(
  ...
  if (imageBuffer?.length) {
    ...
    try {
      const result = await enrichStoreProductImage(product);
      imageAutoGenerated = !result.skipped;
    } catch (err) {
      console.error("[catalog-admin] auto-image failed:", err.message);
    }
  }

  await saveMaster(products);
  schedulePublish(lastCatalogAdminChatId);
  return { product, updated, imageAutoGenerated };
}
...
export async function handleCatalogCommand(adminChatId, text) {
  ...
  if (/^#add\b/i.test(t)) {
    ...
    const { product, updated, imageAutoGenerated } = await upsertStoreProduct(draft, null, {
      matchMode: UPSERT_MATCH.ALWAYS_ADD,
    });
    rememberTextCatalogProduct(adminChatId, product.id);
    await sendAdminOnlyText(
      adminChatId,
      formatProductAck(product, { updated, awaitingPhoto: true, imageAutoGenerated })
    );
    return true;
  }
